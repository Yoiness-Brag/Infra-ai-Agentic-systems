...
## 20. End-to-end finalization pass (2026-08-10)

A 7-agent read-only review of every file under `infra-code/` (plus `docs/`, `platform/`,
`backend-agent-docs/`, `evals/`) produced ~120 findings, logged in
`../.backlog/e2e-finalization-register.md`. That review also **re-verified §19's claims and found
only 6 of the 14 D-register items were actually fixed** (D-04, D-07, D-08, D-09, D-10, D-12 were
still open; D-02 and D-03 were partial). This section records what changed. Where it conflicts with
§1–§19, this section wins.

### 20.1 Scope added beyond the original MVP

| Capability | Implementation |
|---|---|
| PDF upload | `POST /documents` (multipart) → MinIO object → Postgres ledger → queued job |
| Object store | **MinIO** as `minio.ai-platform:9000`, bucket `documents`, console on `localhost:9001` |
| Document knowledge | Ingest worker extracts text, chunks per page, writes Graphiti episodes into `<app>:<subject>:doc:<id>` |
| LangGraph | `recall → delegate → persist` graph over the A2A call |
| Checkpoints | **FalkorDB**, via `langchain-falkordb`'s `FalkorDBSaver` (graph `langgraph_checkpoints`) |
| Logs | **Loki 3.7.6** + **Grafana Alloy 1.18.1** (API-based pod tailing, no hostPath) |
| Autoscaling | **KEDA 2.20.2** + `ScaledObject` on `agent_backend_in_flight_requests` |
| Tracing sink | Alloy OTLP receiver → Langfuse (opt-in profile) |
| Swagger | `/docs`, `/openapi.json`, `/redoc` routed through Kong with **no** `jwt` plugin; `POST /chat/sync` added because Swagger UI cannot render SSE |

### 20.2 Checkpoint backend — why not `langgraph-checkpoint-redis`

FalkorDB is a Redis **module**, and its image loads only `falkordb.so`. `langgraph-checkpoint-redis`
requires **RedisJSON** and **RediSearch**: `setup()` issues `FT.CREATE` and every write issues
`JSON.SET`. Neither command exists on FalkorDB, so that package cannot work — no configuration fixes
it, and adding a second Redis would be the duplicate-stack anti-pattern. FalkorDB's own
`langchain-falkordb` ships `FalkorDBSaver`, which stores checkpoints as Cypher nodes. Verified at
build time: it subclasses `BaseCheckpointSaver` and implements every required sync and async method.
`CHECKPOINT_BACKEND=memory` falls back to `InMemorySaver` if it ever fails to load.

### 20.3 Corrections to earlier sections

- **§4 / §7** — Kong route `chat` had `strip_path: true` against a service URL with no path, so Kong
  proxied `POST /` upstream and **every gateway request 404'd**. Now `strip_path: false`.
- **§7** — the `jwt` plugin was service-scoped, which would have 401'd `/docs`. It is now route-scoped.
- **§6** — `gemini-api` is required in **three** namespaces (`kagent`, `ai-platform`, `ai-gateway`);
  §6 listed two, and Kong mounts it, so Kong could never start.
- **§16 / §19 (D-03)** — confirmed via Kong's schema: `jwt_secrets.secret` is `encrypted`, **not**
  `referenceable`, so `{vault://env/...}` cannot work for it. The declarative config is now rendered
  by `envsubst` into a **Secret** (`kong-declarative`, key `kong.yml`) consumed via
  `dblessConfig.secret`. It is no longer a ConfigMap, is no longer committed, and ArgoCD can no
  longer revert it. `platform/kong/README.md`'s vault claim was wrong and has been corrected.
- **§10** — kagent chart `0.6.21` → **`0.9.12`**. The chart enables **13 built-in agents, a UI, kmcp,
  querydoc and grafana-mcp by default** (~3–4 GiB). All are disabled; only the controller and its
  bundled Postgres remain (**384 MiB**). The UI has no `enabled` flag — `ui.replicas: 0` is the lever.
- **§10** — the `kagent` namespace was default-deny in both directions with **no in-namespace rule**,
  so the controller could never reach its own bundled Postgres. Fixed.
- **§10** — `memory_search` / `memory_add_episode` were removed from the Agent. Both require a
  `group_id` the model had no way to know, so it would have invented one and read or written another
  session's partition. The backend owns memory; this also removes the double-write.
- **§13** — the eval judge metrics are `{correctness, faithfulness}`; the platform's five are
  `{hallucination, helpfulness, relevancy, conciseness, toxicity}`. Only `faithfulness` overlaps.
- **§17** — the A2A well-known path is **`/.well-known/agent-card.json`** (renamed in A2A v0.3.0,
  IANA-registered in v1.0). kagent still serves the legacy `agent.json`; the client tries the new
  path first and falls back. kagent 0.9.12 speaks spec **v1.0** with a v0.3 compat shim, so the
  client pins `A2A-Version: 0.3` explicitly and warns if the Agent Card disagrees.

### 20.4 Resource budget (the binding constraint)

The k3d node is capped (`serversMemory`, default `8g`; `K3D_MEMORY` overrides) with
`system-reserved=512Mi` and `eviction-hard=memory.available<400Mi`. Without this the kubelet
advertised the host's full RAM and the **host** OOM killer arbitrated overcommit instead of Kubernetes.

| Profile | Memory requests | Fits |
|---|---|---|
| `make up-direct` (no ArgoCD) | ~4.5 GiB | 6 GiB node |
| `make up` (with ArgoCD) | ~5.1 GiB + ~0.7 GiB system | 8 GiB node |
| `+ make langfuse-up` | **+2.9 GiB** | needs `K3D_MEMORY=11g` and ~11 GiB free host RAM |

Langfuse v3 **requires ClickHouse** — there is no Postgres-only path — so it is an opt-in profile.
Postgres, FalkorDB and MinIO run at Guaranteed QoS (`requests == limits`); previously every pod was
Burstable, which ranked the two data stores *first* for eviction.

### 20.5 Known gaps that remain

- `make up` (GitOps) needs the repo pushed to a reachable remote; ArgoCD clones over the network.
  `make up-direct` is the offline path.
- No TLS at the edge, no cert-manager, no OPA Gatekeeper, no NATS, no Qdrant, no rerank-svc,
  no CubeSandbox. Reranking and the RAGAS gates are therefore still unmet.
- Document retrieval is graph-only (no hybrid dense+sparse, no RRF).
- The Gemini embedder in `memory.py` still egresses directly rather than through Kong, so its tokens
  do not appear in Kong's AI metrics. Egress is scoped by `ipBlock` rather than wide-open.
- Langfuse's chart pulls `bitnamilegacy/*` images, which are unmaintained since Aug 2025 and will
  fail a strict Trivy gate.
