...
`make preflight` checks these and refuses to continue if any is missing.

- **Docker with container egress.** Verify: `docker run --rm alpine:3.20 ping -c2 1.1.1.1`.
  If that fails, check `/etc/docker/daemon.json` for `"iptables": false` — that setting stops Docker
  creating NAT rules, so no container (including every k3d node) can reach the internet. The cluster
  then cannot pull images and the agent cannot reach the Gemini API.
- **CLIs:** `k3d kubectl helm kustomize argocd yq envsubst openssl python3`.
- **RAM:** ~6 GiB free for the core profile, ~11 GiB to add Langfuse. `make preflight` warns below budget.

### 1. Credentials

All secrets live in `.env/.env.dev` (gitignored, never applied by ArgoCD). Only `GOOGLE_API_KEY`
must be a real value; the rest can be generated. `make require-env` fails closed on any empty one.

### 2. Bring it up

```bash
make up-direct     # helm + kustomize in wave order, no git remote needed
# or
make up            # GitOps: installs ArgoCD, applies the App-of-Apps root
```

`make up` requires this repo to be pushed to the `repoURL` in `platform/argocd/applications/*.yaml`,
because ArgoCD's repo-server clones over the network. Use `up-direct` offline.

Tight on RAM? `make up-direct K3D_MEMORY=6g` skips ArgoCD and fits in ~4.5 GiB of requests.

### 3. Verify

```bash
make verify        # fails on the first broken layer
make urls          # every UI endpoint
make smoke         # POST /chat and /chat/sync through Kong with a fresh JWT
```
...
curl -sS -X POST http://localhost:8080/documents \
  -H "Authorization: Bearer $TOKEN" -F file=@yourfile.pdf
# -> {"document_id":"...","job_id":"...","state":"queued"}

curl -sS http://localhost:8080/documents/<document_id> -H "Authorization: Bearer $TOKEN"
# -> {"state":"completed","chunks_total":42,"chunks_done":42}
```

Ingestion is asynchronous: a 200-page PDF is hundreds of sequential Gemini extraction calls, far
past any HTTP timeout. Jobs are claimed from Postgres with `FOR UPDATE SKIP LOCKED`, so they survive
a pod restart and are safe across replicas. Once ingested, the content is recalled automatically on
the next `/chat` turn.

### 6. Langfuse (opt-in — needs more RAM)

```bash
make down && make up-direct K3D_MEMORY=11g && make langfuse-up
```

Langfuse v3 hard-requires ClickHouse (no Postgres-only mode), so the profile costs ~2.9 GiB on top
of the core stack. It is deliberately not part of the default bring-up.

### 7. Tear down

```bash
make down    # delete the cluster; PVC data survives under ~/.k3d-infra-ai-mvp/storage
make nuke    # delete the cluster AND its persisted volumes
```

### Useful targets

`make help` lists everything. Most used: `preflight`, `secrets`, `verify`, `urls`, `token`, `smoke`,
`lint`, `argocd-password`, `argocd-login`, `argocd-status`.
