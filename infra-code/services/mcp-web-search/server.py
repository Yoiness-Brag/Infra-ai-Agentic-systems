...
MAX_SNIPPET_CHARS = int(os.getenv("MAX_SNIPPET_CHARS", "600"))
MAX_URL_CHARS = 2048
...
def sanitize(text: str, limit: int) -> str:
    ...
    Search results are untrusted web content that lands directly in the agent's
    context (OWASP LLM-01). Full defence belongs at the gateway, but three cheap
    measures remove the easy vectors: strip control characters used to smuggle
    fake role markers, collapse newlines so injected text cannot fake a new
    turn, and hard-cap length so one result cannot flood the context window.
    """
    if not isinstance(text, str):
        return ""
    cleaned = text.translate(_CONTROL_CHARS).replace("\r", " ").replace("\n", " ")
...
def sanitize_url(url: str) -> str:
    """Keep only plain http(s) URLs; drop javascript:/data: and oversized values."""
    if not isinstance(url, str):
        return ""
    candidate = url.strip()[:MAX_URL_CHARS]
    if not candidate.lower().startswith(("http://", "https://")):
        return ""
    return candidate.translate(_CONTROL_CHARS)
...
def web_search(query: str, max_results: int = DEFAULT_MAX_RESULTS) -> list[dict]:
    ...
    try:
        raw = _search_ddgs(query, bounded)
    except Exception as exc:
        logger.warning("ddgs search failed (%s); using httpx fallback", exc)
        try:
            raw = _search_httpx(query, bounded)
        except Exception as exc2:
            logger.error("httpx fallback search failed: %s", exc2)
            return []

    results: list[dict] = []
    for hit in raw[:bounded]:
        url = sanitize_url(hit.get("url", ""))
        if not url:
            continue
        results.append(
            {
                "title": sanitize(hit.get("title", ""), MAX_TITLE_CHARS),
                "url": url,
                "snippet": sanitize(hit.get("snippet", ""), MAX_SNIPPET_CHARS),
            }
        )
    return results
