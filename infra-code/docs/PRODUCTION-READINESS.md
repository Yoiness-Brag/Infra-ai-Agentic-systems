...
## 7. Status after the 2026-08-10 finalization pass

The D-register above was **re-verified** file by file. Section 19 of `SPEC.md` claimed all
run-blockers were closed; that was not accurate. Actual state found, and state now:

| ID | Claimed in §19 | Verified state | State now |
|---|---|---|---|
| D-01 kagent NetworkPolicy | fixed | fixed, but **had no in-namespace rule**, so the controller could never reach its own bundled Postgres | **fixed** |
| D-02 netpol selectors | fixed | partial — dual labels never applied | **fixed** (dual labels on all workload pods) |
| D-03 Kong secrets | fixed | partial — `${JWT_SECRET}` still literal in the committed ConfigMap | **fixed** (rendered into a Secret, out of git) |
| D-04 ArgoCD Helm apps | open | open | **fixed** (`11-kong-helm`, `19-kagent-crds`, `20-kagent-helm`, `05-keda`) |
| D-05 MCP writable /tmp | fixed | fixed for MCPs, **never applied to agent-backend** despite this doc saying "mirror agent-backend" | **fixed** |
| D-06 Graphiti timeouts | fixed | fixed | fixed |
| D-07 SHA-pin CI | open | open; `trivy-action@0.36.0` did not even exist (real tag is `v0.36.0`) | **fixed** (11/11 pinned) |
| D-08 FalkorDB persistence | open | open | **fixed** (AOF + save points + maxmemory) |
| D-09 session takeover | open | open | **fixed** (subject+app guard, 403) |
| D-10 graphiti Dockerfile | open | open, and both MCP images `pip install`ed a **hardcoded** dep list divergent from `pyproject.toml` | **fixed** (2-stage uv + lockfiles) |
| D-11 graphiti-core floor | fixed | fixed | fixed |
| D-12 Kong seccomp | open | open — and container-only fields sat in the **Pod**SecurityContext, so the API server dropped them | **fixed** |
| D-13 memory README model | fixed | fixed | fixed |
| D-14 SPEC ledger honesty | fixed | fixed | extended by SPEC §20 |

### New P0s found in this pass (none were in the original register)

1. Kong route `chat` had `strip_path: true` against a path-less service URL — **every gateway
   request 404'd**. The entire happy path was broken.
2. `extraEnvVars` is not a key in the `kong/kong` chart; Helm ignored it silently, so `JWT_SECRET`
   and `GOOGLE_API_KEY` never reached Kong.
3. `gemini-api` was never created in `ai-gateway`, so Kong would have hung in
   `CreateContainerConfigError`.
4. kagent chart 0.9.12 enables **13 built-in agents + UI + kmcp + querydoc + grafana-mcp** by
   default — roughly 3–4 GiB on a host with ~6 GiB free.
5. The `kagent` namespace had no in-namespace NetworkPolicy allow (see D-01 above).
6. The A2A circuit breaker gated `/readyz`, which at one replica is a **permanent deadlock**: the
   pod leaves the Service, so no request can ever close the breaker.
7. FalkorDB probes sent `AUTH ""` on the documented no-password path, which the server rejects
   before `PING` — an infinite restart loop.
8. The k3d node had no memory cap, so the kubelet advertised the host's full RAM and the **host**
   OOM killer arbitrated overcommit instead of Kubernetes.

### Still open (deliberately)

TLS/cert-manager, OPA Gatekeeper, NATS, Qdrant, rerank-svc (so **reranking and the RAGAS gates
remain unmet**), CubeSandbox, Argo Rollouts, ArgoCD Image Updater, and the four-tool eval harness.
Full list in `SPEC.md` §20.5 and `../.backlog/e2e-finalization-register.md`.
